...
void main() {
  group(r'\epsilon\frac 1 3', () {
    ...
    test('correctly passes token to symbol', () {
      expect(((rootNode as GroupNode).children[0] as SymbolNode).context.input, equals(r'\epsilon'));
    });
    test('recognizes function', () {
      expect((rootNode as GroupNode).children[1], isA<FunctionNode>());
    });
    test('correctly passes token to function', () {
      expect(((rootNode as GroupNode).children[1] as FunctionNode).context.input, equals(r'\frac'));
    });
    test('correctly passes two nodes to frac', () {
      expect(((rootNode as GroupNode).children[1] as MultiChildNode).children, hasLength(2));
    });
  ...
  group(r'\sf{\bf{\textcolor{red}s}}', () {
    ...
    test('succeeds with a single node (function) as the root node', () {
      expect(
        () => rootNode = Parser(r'\sf{\bf{\textcolor{red}s}}').parse() as FontNode,
        returnsNormally,
      );
    });
  ...
  group(r'\varepsilon = \frac{\frac{2}{1}}{3}', () {
    ...
    test('succeeds with group root node', () {
      expect(() => rootNode = Parser(r'\varepsilon = \frac{\frac{2}{1}}{3}').parse() as GroupNode,
          returnsNormally);
    });
